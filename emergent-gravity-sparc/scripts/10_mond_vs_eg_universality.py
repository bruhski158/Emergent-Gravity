...
def main():
    ...
    for filepath in processed_files:
        ...
        if np.any(valid_X):
            ...
            physically_meaningful = g_obs[valid_X] > g_bar[valid_X]
            
            valid_eg_bounds = (a0_eg_local > 1e-13) & (a0_eg_local < 1e-8) & physically_meaningful
            r_all_eg.extend(r_kpc[valid_X][valid_eg_bounds])
            a0_all_eg.extend(a0_eg_local[valid_eg_bounds])
            
        physically_meaningful_mond = g_obs > g_bar
        a0_mond_local = (g_obs * (g_obs - g_bar)) / g_bar
        valid_mond_bounds = (a0_mond_local > 1e-13) & (a0_mond_local < 1e-8) & physically_meaningful_mond
        r_all_mond.extend(r_kpc[valid_mond_bounds])
        a0_all_mond.extend(a0_mond_local[valid_mond_bounds])
    ...
    global_median_eg = np.median(a0_all_eg)
    global_median_mond = np.median(a0_all_mond)
    
    print(f"Emergent Gravity:")
    print(f"  Global Median a_0: {global_median_eg:.2e}")
    print(f"  Variance from Theory: {abs(global_median_eg - A0_EXPECTED)/A0_EXPECTED * 100:.1f}%\n")
    
    print(f"MOND:")
    print(f"  Global Median a_0: {global_median_mond:.2e}")
    print(f"  Variance from Theory: {abs(global_median_mond - A0_EXPECTED)/A0_EXPECTED * 100:.1f}%\n")
    
    print("Generating side-by-side Universality plot...")
    
    fig, axes = plt.subplots(1, 2, figsize=(16, 7), sharey=True)
    
    axes[0].scatter(r_all_mond, a0_all_mond, alpha=0.1, s=15, color='blue', edgecolors='none', label='Local $a_0$ Data Points')
    ...
    axes[0].legend(loc='lower left', fontsize=12, framealpha=0.9)
    
    axes[1].scatter(r_all_eg, a0_all_eg, alpha=0.1, s=15, color='purple', edgecolors='none', label='Local $a_0$ Data Points')
    ...
    axes[1].axhline(A0_EXPECTED, color='red', linestyle='--', linewidth=3, label='Universal $a_0$ Target')
    ...
    plt.tight_layout()
    save_path = os.path.join(project_root, "results", "figures", "10_mond_vs_eg_universality.png")
